...
# 3. 찾아보기: 저장 폴더를 선택하면 텍스트 위젯 입력
# 4. 가로넓이: 이미지 넓이 지정( 원본유지, 1024, 800, 640)
# 5. 간격 : 이미지 간의 간격 지정(없음 넓게 보통 작게)
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일 선택하세요", \
        ...
        initialdir=r"C:\Users\hyj\Desktop\PythonWorkspace\Python_basic") # 최초에 사용자가 지정한 경로 보여줌
...
def merge_image():
    ...
    dest_path = os.path.join(txt_dest_path.get(), "YJ_photo.jpg")
