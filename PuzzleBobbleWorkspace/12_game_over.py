...
class Pointer(pygame.sprite.Sprite):
    ...
    def draw(self, screen):
        screen.blit(self.image, self.rect)
        # pygame.draw.circle(screen, RED, self.position, 3) #빨간 중심점
...
def remove_adjacent_bubbles(row_idx, col_idx, color):
    ...
    if len(visited) >= 3: # 같은 색깔 버블이 3개이상있으면
...
def visit(row_idx, col_idx, color=None):
    ...
    if color and map[row_idx][col_idx] != color:
        return
    # 버블이 빈공간이거나, 버블이 존재할 수 없는 위치인지 확인
...
def draw_bubbles():
    ...
    if curr_fire_count == 2:
        to_x = random.randint(0,2) - 1 # random.randint(-1,1)도 가능 # -1 ~ 1
